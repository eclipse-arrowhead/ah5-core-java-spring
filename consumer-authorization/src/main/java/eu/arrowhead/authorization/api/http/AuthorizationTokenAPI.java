...
public class AuthorizationTokenAPI {
	...
	@GetMapping(path = AuthorizationConstants.HTTP_API_OP_VERIFY_PATH, produces = MediaType.APPLICATION_JSON_VALUE)
	public @ResponseBody AuthorizationTokenVerifyResponseDTO verify(final HttpServletRequest httpServletRequest, @PathVariable(required = true) final String token) {
		...
		final String origin = HttpMethod.GET.name() + " " + AuthorizationConstants.HTTP_API_AUTHORIZATION_TOKEN_PATH + AuthorizationConstants.HTTP_API_OP_VERIFY_PATH.replace(AuthorizationConstants.HTTP_PATH_PARAM_TOKEN, token);
		final String requesterSystem = HttpUtilities.acquireName(httpServletRequest, origin);

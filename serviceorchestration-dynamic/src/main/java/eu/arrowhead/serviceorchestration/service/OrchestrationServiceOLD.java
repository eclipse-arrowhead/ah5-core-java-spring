...
@Service
public class OrchestrationServiceOLD {
	...
	public OrchestrationResponseDTO pull(final OrchestrationForm form) {
		...
		if (!resultsAreInterCloud) {
			if (!temporaryLock) {
				candidates = orchServiceUtils.filterOutReservedOnes(candidates); // -> LOCK LÉPÉS IDE
				if (Utilities.isEmpty(candidates) && !form.hasFlag(OrchestrationFlag.ALLOW_INTERCLOUD)) {
					return emptyResult;
				}
			}
		...
		if (!resultsAreInterCloud) {
			orchServiceUtils.markIfTranslationIsNeeded(candidates); // -> Valójában tudom, ha csak interface helyesek vannak a listában
		...
		if (form.hasFlag(OrchestrationFlag.MATCHMAKING)) {
			...
			// Rerserválni és a többit elereszteni
		}

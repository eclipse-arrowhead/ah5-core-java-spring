...
public class OrchestrationHistoryManagementValidation {
	...
	//-------------------------------------------------------------------------------------------------
	public void validateQueryService(final OrchestrationHistoryQueryRequestDTO dto, final String origin) {
	...
	public OrchestrationHistoryQueryRequestDTO validateAndNormalizeQueryService(final OrchestrationHistoryQueryRequestDTO dto, final String origin) {
		...
		try {
			...
			normalized.targetSystems().forEach(sys -> {
				nameValidator.validateName(sys);
			});

			normalized.serviceDefinitions().forEach(def -> {
				nameValidator.validateName(def);
			});
